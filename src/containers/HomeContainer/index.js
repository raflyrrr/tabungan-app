...
class HomeContainer extends Component {
  ...
  async fetchData() {
    ...
    const totalExpense = this.calculateTotal(transactionData, 'Pengeluaran')
    const totalIncome = this.calculateTotal(transactionData, 'Pemasukan')
      ...
      <View style={{ height }}>
        ...
        {transactionData.length > 0 ?
          ...
          : <View style={{ height, justifyContent: 'center', alignItems: 'center' }}>
            <Text>Silahkan pilh bulan untuk melihat transaksi</Text>
          </View>
        }
        ...
        >
          <IonIcons name={Icons.IonIcons.calender} size={25} color="#fff" />
        </TouchableOpacity>
      </View>
    );
  }
  ...
  return (
    <LineargradientAnimation style={[styles.homeContainerHeaderView, { transform: [{ translateY }], }]}
      ...
      <View style={{ flex: 2, justifyContent: 'center', alignItems: 'center', }}>
        <View>
          <Animated.Text style={{ color: '#fff', fontWeight: '600', opacity: textOpacity }}>Tabungan Anda</Animated.Text>
        </View>
        <Animated.View style={{ transform: [{ translateY: balanceTextMargin }] }}>

          <View style={{ justifyContent: 'center', alignItems: 'center', flexDirection: 'row' }}>
            <NumberFormat value={totalBalance} displayType={'text'} decimalSeparator="," thousandSeparator="." prefix={'Rp'} renderText={value => <Text style={[styles.homeContainerHeaderCurrentBalanceText]}>{value}</Text>}/>
          </View>
        </Animated.View>
        <View>
      ...
      <View style={{ flex: 1, flexDirection: 'row', marginHorizontal: 15, }}>
        <View style={{ flex: 3, }}>
          <Info
            ...
            opacity={textOpacity}
          />
          <NumberFormat value={totalIncome} displayType={'text'} decimalSeparator="," thousandSeparator="." prefix={'Rp'} renderText={value => <Text style={[styles.homeContainerHeaderCurrentIncomeText]}>{value}</Text>}/>
        ...
        <View style={{ flex: 2}}>
          <Info
            ...
            opacity={textOpacity}
          />
        <NumberFormat value={totalExpense} displayType={'text'} decimalSeparator="," thousandSeparator="." prefix={'Rp'} renderText={value => <Text style={[styles.homeContainerHeaderCurrentExpenseText]}>{value}</Text>}/>
        </View>
      </View>
    ...
    >
      <View style={styles.transactionDetailModalView}>
        ...
        <View style={styles.transactonDetailModalViewContent}>
          ...
          <TransactionDetailContent
            type="Jumlah"
          /><NumberFormat value={amount} displayType={'text'} decimalSeparator="," thousandSeparator="." prefix={'Rp'} renderText={value => <Text style={[styles.transactionDetailClick]}>{value}</Text>}/>       
